#!/usr/bin/env python3
"""
Merge crypto hourly price JSON files into a single JSONL file with automatic symbol fixing.

This script:
1. Merges individual crypto hourly price JSON files into crypto_hourly_merged.jsonl
...
Usage:
    python merge_crypto_jsonl_hourly.py

The individual crypto files should be located in the 'coin/' subdirectory
and follow the naming pattern: hourly_prices_{SYMBOL}.json
"""
...
crypto_symbols_usdt = [
    ...
    "SOL",   # Solana/USDT
    "ADA",   # Cardano/USDT
    # "SUI",   # Sui/USDT - Removed due to no USDT market data
    "LINK",  # Chainlink/USDT
    "AVAX",  # Avalanche/USDT
    # "LTC",   # Litecoin/USDT - Removed due to no USDT market data
    "DOT",   # Polkadot/USDT
]

def backup_crypto_data():
    """Create a backup of the existing crypto_hourly_merged.jsonl file if it exists"""
...
# Merge all crypto hourly price JSON files, write one line per file to crypto_hourly_merged.jsonl
current_dir = os.path.dirname(__file__)
assert (Path(current_dir) / "coin").exists(), "coin/ directory not found!"
pattern = os.path.join(current_dir, "coin", "hourly_prices_*.json")
files = sorted(glob.glob(pattern))
assert files, "No crypto hourly price files found to merge!"
output_file = os.path.join(current_dir, "crypto_hourly_merged.jsonl")

print(f"Found {len(files)} crypto hourly files to merge")
        ...
        try:
            ...
            if isinstance(series, dict) and series:
                ...
                if isinstance(meta, dict):
                    meta["1. Information"] = "Hourly Prices (buy price, high, low, sell price) and Volumes"
...
print(f"\nCrypto hourly merge complete! Output saved to: {output_file}")
