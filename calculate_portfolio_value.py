...
def calculate_portfolio_value(modelname: str, date: Optional[str] = None, market: str = "cn") -> Dict:
    """
    计算投资组合的总市值
    ...
    for symbol, shares in stock_positions.items():
        ...
        if price is not None:
            value = shares * price
            stock_values[symbol] = {"shares": shares, "price": price, "value": value}
            total_stock_value += value
        else:
            print(f"⚠️  Warning: No price found for {symbol} on {date}")
            stock_values[symbol] = {"shares": shares, "price": None, "value": 0.0}
